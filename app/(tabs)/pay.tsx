...
export default function PayScreen() {
  ...
  const pollForPaymentCompletion = async (continueToken: string, continueUri: string, quoteId: string, paymentData: any, attempts: number = 0) => {
    ...
    try {
      ...
      const response = await fetch('http://196.42.89.43:3001/api/payment/complete', {
        method: 'POST',
  ...
  const processOpenPayment = async (paymentData: any) => {
    try {
      console.log('🔄 Starting Open Payments flow...');
             ...
             // Step 1: Create quote
       const quoteResponse = await fetch('http://196.42.89.43:3001/api/payment/quote', {
         method: 'POST',
         headers: { 'Content-Type': 'application/json' },
         body: JSON.stringify({
             ...
             }},
             {
              ...
              try {
                console.log('🔄 Processing payment...');
                                 ...
                                 // Step 3: Send payment
                 const paymentResponse = await fetch('http://196.42.89.43:3001/api/payment/send', {
                   method: 'POST',
                   headers: { 'Content-Type': 'application/json' },
                   body: JSON.stringify({
