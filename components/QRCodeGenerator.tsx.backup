...
export default function QRCodeGenerator({ onQRGenerated }: QRCodeGeneratorProps) {
  ...
  const generateQRCode = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://192.168.10.56:3001/api/qr/generate`, {
        method: 'POST',
        headers: {
  ...
  const shareQRCode = async () => {
    ...
    try {
      const shareMessage = `💰 Payment Request\n\nAmount: ${currency} ${amount}\nDescription: ${description}\n\nScan the QR code to pay via Dinela app using Open Payments.`;
    ...
    return (
      <View style={styles.generatedContainer}>
        ...
        <Text style={styles.instructions}>
          🔍 Customer should scan this QR code with their Dinela app to make payment via Open Payments network.
        </Text>
      </View>
    );
  }
